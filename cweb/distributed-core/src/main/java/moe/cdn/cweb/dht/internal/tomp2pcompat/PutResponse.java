...
public interface PutResponse {
    /**
     * Finish the future and set the keys that have been stored. Success or failure is
     * determined if the communication
     * was successful. This means that we need to further check if the other peers have
     * denied the storage (e.g., due to
     * no storage space, no security permissions). Further evaluation can be retrieved with
     * {@link #avgStoredKeys()}
     ...
     */
    void storedKeys(Map<PeerAddress, Map<Number640, Byte>> rawResult, FutureDone<Void>
            futuresCompleted);
    ...
    /**
     * Checks if the minimum of expected results have been reached. This flag is also used
    ...
    /**
     * Returns the keys that have been stored or removed after evaluation. The evaluation
     * gets rid of the PeerAddress
     * information, by either a majority vote or cumulation. Use {@link FuturePut#evalKeys()}
